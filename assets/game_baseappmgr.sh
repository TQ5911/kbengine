...
function start_gameapp(){
    echo "baseappmgr start....."
    cid=`expr 500000 + $INST_ID`
    $KBE_BIN_PATH/baseappmgr --cid=$cid --instId=$INST_ID 2>&1 > /dev/null &
    echo "baseappmgr start done"
}

function stop_gameapp(){
    echo "baseappmgr stop...."
    pid=`cat $KBE_ROOT/assets/baseappmgr_${INST_ID}.pid`
    kill $pid
