...
function start_gameapp(){
    echo "machine start....."
    cid=`expr 100000 + $INST_ID`
    $KBE_BIN_PATH/machine --cid=$cid --instId=$INST_ID 2>&1 > /dev/null &
    echo "machine start done"
}

function stop_gameapp(){
    echo "machine stop...."
    pid=`cat $KBE_ROOT/assets/machine_${INST_ID}.pid`
    kill $pid
