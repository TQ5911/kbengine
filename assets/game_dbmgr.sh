...
function start_gameapp(){
    echo "dbmgr start....."
    cid=`expr 400000 + $INST_ID`
    $KBE_BIN_PATH/dbmgr --cid=$cid --instId=$INST_ID 2>&1 > /dev/null &
    echo "dbmgr start done"
}

function stop_gameapp(){
    echo "dbmgr stop...."
    pid=`cat $KBE_ROOT/assets/dbmgr_${INST_ID}.pid`
    kill $pid
