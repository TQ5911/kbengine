...
function start_gameapp(){
    echo "cellappmgr start....."
    cid=`expr 600000 + $INST_ID`
    $KBE_BIN_PATH/cellappmgr --cid=$cid --instId=$INST_ID 2>&1 > /dev/null &
    echo "cellappmgr start done"
}

function stop_gameapp(){
    echo "cellappmgr stop...."
    pid=`cat $KBE_ROOT/assets/cellappmgr_${INST_ID}.pid`
    kill $pid
