...
function start_gameapp(){
    echo "cellapp start....."
    cid=`expr 800000 + $INST_ID`
    $KBE_BIN_PATH/cellapp --cid=$cid --instId=$INST_ID 2>&1 > /dev/null &
    echo "cellapp start done"
}

function stop_gameapp(){
    echo "cellapp stop...."
    pid=`cat $KBE_ROOT/assets/cellapp_${INST_ID}.pid`
    kill $pid
