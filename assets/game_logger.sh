...
function start_gameapp(){
    echo "logger start....."
    cid=`expr 200000 + $INST_ID`
    $KBE_BIN_PATH/logger --cid=$cid --instId=$INST_ID 2>&1 > /dev/null &
    echo "logger start done"
}

function stop_gameapp(){
    echo "logger stop...."
    pid=`cat $KBE_ROOT/assets/logger_${INST_ID}.pid`
    kill $pid
