...
class Branch(BaseFlow):
    ...
    def __init__(self, event_id, controller=None, **kwargs):
        super().__init__(event_id, controller, **kwargs)
        self.regr_flow_handler(self._branch_handler, 1)

    def _branch_handler(self, this, src_e, obj, **ref_params):
        if self.CONDITION_KEY not in ref_params:
            raise TypeError('must bind condition first')

        cond_result  = ref_params[self.CONDITION_KEY]
    ...
    def bind_condition(self, condition, key):
        self.ref_param(condition, key, self.CONDITION_KEY)

    def unbind_condition(self):
        self.unref_param(self.CONDITION_KEY)

    def rebind_condition(self, new_condition, key):
        self.unref_param(self.CONDITION_KEY)
        self.ref_param(new_condition, key, self.CONDITION_KEY)
...
class DoN(BaseFlow, OptionInputParamMixin):
    ...
    def __init__(self, event_id, controller=None, **kwargs):
        super(DoN, self).__init__(event_id, controller, **kwargs)
    ...
    def _handle_enter_input(self, this, src_e, obj, **ref_params):
        if self._looping_count >= self.get_loop_n():
            return
        self._looping_count += 1
        self.trigger_all(self.BINDING_OUTPUT, obj)

    def _handle_reset_input(self, this, src_e, obj, **ref_params):
        self._looping_count = 0
...
class FlipFlop(BaseFlow):
    ...
    def __init__(self, event_id, controller=None, **kwargs):
        super(FlipFlop, self).__init__(event_id, controller, **kwargs)
        self._is_a = False
        self.add_param(self.PARAM_IS_A_KEY, self.is_a)
        self.regr_flow_handler(self._handle_flipflop, 1)

    def _handle_flipflop(self, this, src_e, obj, **ref_param):
        if not self._is_a:
...
class BaseForLoop(BaseFlow, OptionInputParamMixin, OverOneTickMixin):
    ...
    def __init__(self, event_id, controller=None, **kwargs):
        super(BaseForLoop, self).__init__(event_id, controller, **kwargs)
        self._looping_counters = {}
        self.regr_flow_handler(self._handle_looping, self.ENTER_LOOP_BODY_IDX)
        self.add_param(self.LOOPING_COUNTERS_KEY, self.get_looping_counters_copy)
    ...
    def get_last_index(self, default=0):
...
class BaseForLoopWithBreak(BaseForLoop):
    ...
    def _handle_break(self, this, src_e, ctx, **ref_params):
    ...
    def _handle_looping(self, this, src_e, ctx, **ref_params):
        if self._looping_breakers.pop(ctx.tid, False):
            ...
            return
        return super()._handle_looping(self, src_e, ctx, **ref_params)
...
class BaseWhileLoop(BaseFlow, OptionInputParamMixin, OverOneTickMixin):
    ...
    def __init__(self, event_id, controller=None, **kwargs):
        super(BaseWhileLoop, self).__init__(event_id, controller, **kwargs)
        self.regr_flow_handler(self._handle_looping, self.ENTER_LOOP_BODY_IDX)
...
class Sequence(BaseFlow):
    ...
    def _handle_sequence(self, this, src_e, ctx, **ref_params):
...
class Gate(BaseFlow, OptionInputParamMixin):
    ...
    def _handle_enter(self, this, src_e, obj, **ref_params):
    ...
    def _handle_open(self, this, src_e, obj, **ref_params):
        self._closed_flag = False

    def _handle_close(self, this, src_e, obj, **ref_params):
        self._closed_flag = True

    def _handle_toggle(self, this, src_e, obj, **ref_params):
        if self._closed_flag is None:
            self._closed_flag = True if self.is_start_closed() else False
