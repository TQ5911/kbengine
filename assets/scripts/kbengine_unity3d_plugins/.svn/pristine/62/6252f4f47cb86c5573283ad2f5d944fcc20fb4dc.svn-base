...
namespace Deps
{
    public class KCP
    {
        ...
        public int Send(byte[] buffer, int offset, int len)
        {
            ...
            if(stream)
            {
                if (snd_queue_.First!=null)
                {
                    var tailSeg = snd_queue_.Last;
                    int oldLen = tailSeg.Value.data.Length;
                    if (oldLen < mss_)
                    {
                        int capacity = (int)mss_ - oldLen;
                        int extend = (len < capacity) ? len : capacity;
                        var seg = new Segment(oldLen + extend);
                        Buffer.BlockCopy(tailSeg.Value.data, 0, seg.data, 0, oldLen);

                        if (buffer != null && len > 0)
                        {
                            Buffer.BlockCopy(buffer, offset, seg.data, oldLen, extend);
                            offset += extend;
                            len -= extend;
                        }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                seg.frg = stream?0:(UInt32)(count - i - 1);
                snd_queue_.AddLast(seg);
                nsnd_que_++;
                len -= size;
            }
