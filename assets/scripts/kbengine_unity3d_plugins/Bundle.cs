namespace KBEngine
{
	...
	*/
	public class Bundle : ObjectPool<Bundle>
    {
		...
		public void send(NetworkInterfaceBase networkInterface, bool checkHello=true)
		{
			fini(true);
			
			if(networkInterface.valid() && (!checkHello || KBEngineApp.app.helloDone))
			{
				for(int i=0; i<streamList.Count; i++)
				{
			...
			else
			{
                if(!networkInterface.valid())
                    ...
                    Dbg.ERROR_MSG("Bundle::send: cannot send data during hello");
            }
        ...
        {
            checkStream(size);
            stream.writeRawStream(v, offset, size);
        }
