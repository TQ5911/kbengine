namespace KBEngine
{
	...
	*/
	public class MemoryStream : ObjectPool<MemoryStream>
    {
        ...
        public uint readBlobToArray(byte[] array, uint index)
        {
            UInt32 size = readUint32();
            Array.Copy(datas_, rpos, array, index, size);
            rpos += (int)size;
            return size;
        }
        ...
        public Vector3 readPackXYZ()
        {
            //UInt32 packed = readUint32();
            //float minf = -256.0f;
            //float x = ((packed & 0x7FF) << 21 >> 21) * 0.25f;
            //float z = ((((packed >> 11) & 0x7FF) << 21) >> 21) * 0.25f;
            //float y = ((packed >> 22 << 22) >> 22) * 0.25f;
            ...
            // 复制指数和尾数
            xPackData.uv |= (data & 0x7fe00000) >> 5;
            zPackData.uv |= (data & 0x000ffc00) << 6;
            yPackData.uv |= (data & 0x000001ff) << 17;
            ...
            // 设置标记位
            xPackData.uv |= (data & 0x80000000);
            zPackData.uv |= (data & 0x00100000) << 11;
            yPackData.uv |= (data & 0x00000200) << 22;


            // 将提取的值转换为float类型
            float x = xPackData.fv;
            float y = yPackData.fv;
            float z = zPackData.fv;
        ...
        public void writeRawStream(byte[] v, UInt16 offset, UInt16 size)
        {
            //int size = v.Length;
            if (size > space())
            {
