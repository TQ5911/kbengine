...
class IMineWarBase(object):
    ...
    def reqSyncGuildData(self):
        ...
        同步帮派数据
        """
        # LOG_INFO('IMineWarBase.reqSyncGuildData called for player:', self.id)
    ...
    def onPlayerGetGuildInfo(self, guildInfo):
        ...
        LOG_INFO('IMineWarBase.onPlayerGetGuildInfo called for player:', self.id, guildInfo)
    ...
    def onMineWarStateSync(self, state, startTime, endTime, mineList, MineRevenueDict):
        ...
        LOG_INFO('Avatar.onMineWarStateSync state:', state, 'startTime:', startTime, 'endTime:', endTime, mineList, MineRevenueDict)
    ...
    def informPlayerStart(self, endTime):
        ...
        矿战开始通知玩家
        """
        LOG_INFO('Avatar.informPlayerStart endTime:', endTime)
    ...
    def onMineWarEndPlayer(self, changeInfo):
        ...
        """
        LOG_INFO('Avatar.onMineWarEndPlayer state:', changeInfo)
        ...
        for info in changeInfo:
            ...
            if oldGuildId == self.guildUUIDBase and newGuildId != oldGuildId and mapId in self.myMineListBase:
                ...
                LOG_INFO('Avatar.onMineWarEndPlayer won mine:', self.id, mapId)
    ...
    def onMineWarLeaveGuild(self):
        ...
        玩家离开帮派时调用
        """
        LOG_INFO('IMineWarBase.onMineWarLeaveGuild called for player:', self.id)
        # gameengine.getGlobalBase('MineWarStub').playerLeaveMineWarGuild(self)

    def onMineWarFlagSync(self, flagStateList):
        """
        矿战旗帜状态同步回调
        """
        LOG_DBG('IMineWarBase.onMineWarFlagSync called for player:', self.id, 'flagStateList:', flagStateList)
    ...
    def getMineWarCollectionTime(self, lineType):
        ...
        # LOG_INFO('iMineWarBase.getMineWarCollectionTime :', self.id, battleMapId, mineTime, limitTime, mineKey)
    ...
    def mineWarPrecheckCollection(self, collectionId):
        ...
        # LOG_INFO('iMineWarBase.mineWarPrecheckCollection :', self.id, collectionId, collectionList, lineType, self.myMineListBase)
        # 矿战宝箱检查
    ...
    def onMineWarCollectionSuccess(self, collectionId, pickTime):
        ...
        if battleMapId > 0:
            ...
            LOG_INFO('iMineWarCell.onMineWarCollectionSuccess :', self.id, 'args:', collectionId, pickTime, battleMapId, self.getDailyData(mineKey, 0))
    ...
    def onMineWarCollectionReward(self, srcType, awardId, awardVal, awardCtx):
        ...
        LOG_INFO('iMineWarCell.onMineWarCollectionReward :', self.id, 'darkIron num:', num)
        if num > 0:
    ...
    def getMineWarFactor(self, collectionId=0):
        ...
        for lineType, lineCfg in MBMA.datas.items():
            if currLineType in lineCfg['sceneList']:
                ...
                break
        # LOG_INFO('iMineWarCell.getMineWarFactor :', self.id, 'lineType:', lineType, 'factor:', factor)
        return factor
    ...
    def reqMineWarInfo(self, exposed):
        ...
        客户端请求矿战信息
        """
        LOG_INFO('iMineWarBase.reqMineWarInfo called for player:', self.id)
        self.getMineWarInfo()
    ...
    def reqMineWarShareBonus(self, exposed, mapId, shareList):
        ...
        客户端请求矿战分享奖励
        """
        LOG_INFO('iMineWarBase.reqMineWarShareBonus called for player:', self.id, 'mapId:', mapId, shareList)
        if self.guildUUIDBase <= 0 or self.guildBox is None:
            return
    ...
    def reqMineWarGuildMemberScore(self, exposed, lineType):
        ...
        客户端请求矿战帮派成员积分
        """
        LOG_INFO('iMineWarBase.reqMineWarGuildMemberScore called for player:', self.id, 'lineType:', lineType)
        if self.guildUUIDBase <= 0 or self.guildBox is None:
            return
    ...
    def reqMineWarGuildOwnerRank(self, exposed, mapId, last):
        ...
        客户端请求矿战帮派占领排名
        """
        LOG_INFO('iMineWarBase.reqMineWarGuildOwnerRank called for player:', self.id, 'mapId:', mapId, last)
    ...
    def reqMineWarGuildPlayerRank(self, exposed, mapId, lastRank):
        ...
        客户端请求矿战个人贡献排名
        """
        LOG_INFO('iMineWarBase.reqMineWarGuildPlayerRank called for player:', self.id, 'mapId:', mapId, lastRank)
        # if not formula.inMineWarScene(self.baseSpaceNo):
        #     return
    ...
    def onMineWarKillCore(self, lineType):
        ...
        矿战摧毁核心回调
        """
        LOG_INFO('iMineWarBase.onMineWarKillCore called for player:', self.id, 'mapId:', lineType)
        if self.guildUUIDBase <= 0 or self.guildBox is None:
            return
        
        if not self.myGuildInfoBase:
            self.addTimerCB(0.2, 'onMineWarKillCore', (lineType), gametimer.TIMER_TAG_MINE_WAR_KILL_CORE_DALAY)
            LOG_INFO('onMineWarKillCore no guild info, retry later:', self.id, self.guildUUIDBase)
            return
    ...
    def reqMineWarFlagHp(self, exposed, mapId):
        ...
        客户端请求矿战荣誉旗帜血量
        """
        # LOG_INFO('iMineWarBase.reqMineWarFlagHp called for player:', self.id, 'mapId:', mapId)
    ...
    def onMineWarGuildOwnerRankBase(self, selfGuildInfo, rankList):
        ...
        def _iter(_datas):
            while _datas:
                ...
                # LOG_INFO('onMineWarGuildOwnerRank send rank to player:', selfGuildInfo, sendList)
    ...
    def onMineWarGuildPlayerRankBase(self, selfGuildInfo, rankList):
        ...
        def _iter(_datas):
            while _datas:
                sendList = _datas[:batchNum]
                _datas = _datas[batchNum:]
                # LOG_INFO('onMineWarGuildPlayerRank send rank to player:', selfGuildInfo, sendList)
                self.client.onMineWarGuildPlayerRank(selfGuildInfo, sendList)
                yield lambda: None
        self._addPacketSendTask(_iter(rankList))

    def sendMineWarState(self):
        self.client.onMineWarStateChanged(gameglobal.mineGlobalData.mineWarState)
