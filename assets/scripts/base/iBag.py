...
class AwardMixin(object):
    ...
    def pickUpItems(self, sourceId, itemId, pickNum):
...
class CoinBillMixin(object):
    ...
    def reqAvatarCoinBill(self, coinType, tabIndex, tabCount):
...
class IBag(AwardMixin, CoinBillMixin, ShareAwardMixin):
    ...
    def reqGetOnlineTimeReward(self, rewardIdx):
        return
    ...
    def reqBindItem(self, bagType, gridId, itemId):
    ...
    def recycleItems(self, bagType, gridId, itemId):
    ...
    @gamedecorator.limitcall(1)
    def recycleMultipleItems(self, bagType, gridIds):
    ...
    @gamedecorator.limitcall(1)
    def reqMultiItemDisassemble(self, gridIdList, uniqueIdList):
        # 要么全部分解，要么都不分解
        ...
        if gridIDCount <= 0 or uniqueIDCount <= 0 or gridIDCount != uniqueIDCount:
            ERROR_MSG('       reqMultiItemDisassemble, args error:', gridIdList, uniqueIdList)
            return
        if self.bagData.isFull():
        ...
        for gridId, uniqueId in zip(gridIdList, uniqueIdList):
            ...
            if itemObj.uniqueId != uniqueId:
                WARNING_MSG('       reqMultiItemDisassemble, itemId mismatch:', itemObj.itemId, gridId, uniqueId)
                return
    ...
    @gamedecorator.limitcall(1)
    def reqItemDisassemble(self, gridId, costItemId, costItemNum):
        DEBUG_MSG('in reqItemDisassemble:', gridId, costItemId, costItemNum)
        if gridId <= 0 or costItemNum <= 0 or costItemId <= 0:
        ...
        awardCtx = self._getAvatarAwardCtx(rewardID, None)
        # 最终分解的产出
        realAwardVal = dropAward.getAwardOne(rewardID, awardCtx)
        for i in range(0, costItemNum):
            awardVal = dropAward.getAwardOne(rewardID, awardCtx)
            # 检查能否塞下
            if not self.canAddWealthVal(srcType, awardVal + realAwardVal):
                break
            realCostItemNum += 1
            realAwardVal += awardVal
        # 一个都无法分解
        if realCostItemNum <= 0:
            DEBUG_MSG('     reqItemDisassemble cannot disassemble item:', gridId, costItemId, costItemNum, realCostItemNum)
            self.onMessagePre(MMD.datas.bagCapacityNotEnough, [])
            return
        
        # 实际分解
        detail = gameclass.AwardDetail(costItemId=costItemId, realCostItemNum=realCostItemNum)
        self.bagData.deductItemsByGrid(self, {gridId: realCostItemNum}, opUUID, srcType, detail)

        DEBUG_MSG('     reqItemDisassemble, realCostItemNum {} awardVal:{}:'.format(realCostItemNum, awardVal))
        self.addWealth(srcType, awardVal, opUUID, detail)
        # 分解不符合预期
    ...
    def addSignInAwards(self, signInDay):
    ...
    def exchangeItem(self, exchangeId, bindNum, unBindNum):
    ...
    def openNChoiceGift(self, gridId, itemId, choiceStr):
        itemObj = self.bagData.getItemObjByGridId(gridId)
    ...
    def replaceExpiredItem(self, itemUid):
        ...
        if gridObj:
            ...
            replaceItem = gridObj.getReplaceItemWhenExpire()
            if replaceItem:
                self.bagData.addItemsToNewGrid(self, replaceItem, uuid, srcType, detail, gridId=gridId)

    def reqShareReward(self, shareChannelId):
    ...
    def reqRandomSynthesis(self, bagType, itemInfoList):
    ...
    def reqUpgradeSynthesis(self, key):
    ...
    def bagExpansion(self, gridNum, gridId, itemId, useNum, opUUID, context):
        DEBUG_MSG("bagExpansion ", gridNum, gridId, itemId, useNum, opUUID, context)
        commonBagCapacity = BagDataSet.datas['commonBagCapacity']['value']
        if self.capacity >= commonBagCapacity:
            WARNING_MSG('   in bagExpansion, reach limit 1:', self.capacity)
            return
