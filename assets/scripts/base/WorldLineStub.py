...
class WorldLineStub(iGlobal.IGlobal, iBaseNoCell.IBaseNoCell, iTimer.ITimer, \
    ...
    def onLineSpaceGone(self, spaceNo, groupOrder):
        ...
        if not self.lineSpaces:
            ...
            gmCommand.doCommandInside(_agent, "$setcachecfg interfaceEnableLogin 0")
    ...
    def onSpaceLineReady(self, spaceNo):
        ...
        iLineStubBase.ILineStubBase.onSpaceLineReady(self, spaceNo)
    ...
    def enterLineSuccess(self, lineNo, box, gbId, succInfo):
        LOG_INFO('enterLineSuccess', lineNo, box.id, succInfo, gbId)
        ...
        if abs(len(_linePlayers) - _linePlayers.lastUpPlayerNum) >= gameconst.WORLD_LINE_UPDATE_WEIGHT_VAL:
            ...
            _spaceVal.lineSpaceBox.cell.updateSpaceWeight(
                utils.calcSpaceWeight(_linePlayers.lastUpPlayerNum, False,
    ...
    def updateLinePlayerInfo(self, lineNo, box, gbId, infoDic):
        super(WorldLineStub, self).updateLinePlayerInfo(lineNo, box, gbId, infoDic)
        _playerVal = self.allPlayers.getPlayer(lineNo, gbId)
        if not _playerVal:
            LOG_ERR('updateLinePlayerInfo:cannot find player:', self.lineType, lineNo, gbId, box.id, infoDic)
            return
        ...
        if 'areaId' in infoDic:
            newVal = infoDic['areaId']
            linePlayers.onPlayerAreaChanged(gbId, newVal)

    def _requestCollectionPostionByCollId(self, collectionId, spaceNo):
        _mColl = self.getCollectionSharedColl(spaceNo, collectionId, default=())
        if not _mColl:
            LOG_WARN("_requestCollectionPostionByCollId:: coll not found", collectionId, spaceNo)
            return (0, None), False

        _mGameEntityId = random.choice(list(_mColl))
        _mGid = utils.parseGidFromGameEntityId(_mGameEntityId)
        _mSGid = str(_mGid)

        m_datas = utils.getDunModuleData(formula.fetchMapId(spaceNo))
        if _mSGid not in m_datas:
            LOG_WARN("_requestCollectionPostionByCollId:: gid not in data", _mSGid, collectionId, spaceNo)
            return (0, None), False

        _mPrm = m_datas[_mSGid]
        _mPos = (_mPrm['PosX'], _mPrm['PosY'], _mPrm['PosZ'])
        return (_mGameEntityId, _mPos), True

    def leaveLine(self, box, gbId, fromSpaceNo, toSpaceNo, toPosition, toDir):
        super(WorldLineStub, self).leaveLine(box, gbId, fromSpaceNo, toSpaceNo, toPosition, toDir)
    ...
    def switchLineSuccess(self, fromLineNo, toLineNo, box, gbId, extraDic):
        super(WorldLineStub, self).switchLineSuccess(fromLineNo, toLineNo, box, gbId, extraDic)
        for _lineNo in (fromLineNo, toLineNo):
            _linePlayers = self.allPlayers.getLinePlayers(_lineNo)
            if abs(len(_linePlayers) - _linePlayers.lastUpPlayerNum) >= gameconst.WORLD_LINE_UPDATE_WEIGHT_VAL:
                _linePlayers.lastUpPlayerNum = len(_linePlayers)
                _spaceVal = self.getLineSpaceVal(_lineNo)
                _spaceVal.lineSpaceBox.cell.updateSpaceWeight(
                    utils.calcSpaceWeight(_linePlayers.lastUpPlayerNum, False,
                    gameconst.EntNumPerPlayerInAOI.worldLine,
                    gameconst.WORLD_LINE_BASE_WEIGHT))
    ...
    def debugPlayerAreaInfo(self):
        ...
        for areaid in iter(wcad.datas):
            _linen = [0] * len(lines)
            for _lineNo in lines:
                linePlayers = self.allPlayers.getLinePlayers(_lineNo)
                areaPlayersDic = linePlayers.playersInArea(areaid)
                num = len(areaPlayersDic)
                if num: LOG_DBG('!!!!!!!        line: {}, num: {}'.format(_lineNo, num))
                _linen[_lineNo] = num
                count += num
    ...
    def notifyPlayerOffline(self, lineNo, gbId):
        ...
        _linePlayersData = self.allPlayers.getLinePlayers(lineNo)
        if not _linePlayersData:
            return

        if abs(len(_linePlayersData) - _linePlayersData.lastUpPlayerNum) >= gameconst.WORLD_LINE_UPDATE_WEIGHT_VAL:
            _linePlayersData.lastUpPlayerNum = len(_linePlayersData)
            _spaceVal = self.getLineSpaceVal(lineNo)
            _spaceVal.lineSpaceBox.cell.updateSpaceWeight(
                utils.calcSpaceWeight(_linePlayersData.lastUpPlayerNum, False,
                gameconst.EntNumPerPlayerInAOI.worldLine,
                gameconst.WORLD_LINE_BASE_WEIGHT))
