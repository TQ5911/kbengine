...
class IWorkshop(object):
    ...
    def reqWorkshopSetAutoMF(self, autoMF):    
        DEBUG_MSG("reqWorkshopSetAutoMF ", autoMF)
        if not gameconfig.enableWorkshop():
            WARNING_MSG("reqWorkshopSetAutoMF ~ workshop is not open")
            self.client.onWorkshopSetAutoMF(gameconst.WorkshopResult.WORKSHOP_LIMIT_FUNC_NOT_OPEN)
            return
        self.autoMF = autoMF
        self.client.onWorkshopSetAutoMF(gameconst.WorkshopResult.WORKSHOP_SUCCESS)

    @gamedecorator.limitcall(1)
    def reqWorkshopMF(self, itemID, batchCount):    
        DEBUG_MSG("reqWorkshopMF ", itemID, batchCount)
        if not gameconfig.enableWorkshop():
            WARNING_MSG("reqWorkshopSetAutoMF ~ workshop is not open")
            self.client.onWorkshopMF(gameconst.WorkshopResult.WORKSHOP_LIMIT_FUNC_NOT_OPEN)
            return
        
        if self.bagData.isFull():
            ERROR_MSG("reqWorkshopMF ~ bag is full", itemID, batchCount)
            self.client.onWorkshopMF(gameconst.WorkshopResult.WORKSHOP_LIMIT_BAG_SPACE, itemID, batchCount)
            return
        
        if self.bagData.isLocked():
            ERROR_MSG("reqWorkshopMF ~ bag is locked", itemID, batchCount)
            self.client.onWorkshopMF(gameconst.WorkshopResult.WORKSHOP_LIMIT_BAG_LOCK, itemID, batchCount)
            return
        
        if not dataUtils.isValidItemId(itemID):
            ERROR_MSG("reqWorkshopMF ~ unknow item id", itemID, batchCount)
            self.client.onWorkshopMF(gameconst.WorkshopResult.WORKSHOP_UNKNOW, itemID, batchCount)
            return
        ...
        if batchLimit <= 0:
            ...
            self.client.onWorkshopMF(gameconst.WorkshopResult.WORKSHOP_LIMIT_ILLEGAL_CONFIG, itemID, batchCount)
            return
        
        if batchCount > batchLimit:
            ERROR_MSG("reqWorkshopMF ~ workShop_config error", itemID, batchCount, batchLimit)
            self.client.onWorkshopMF(gameconst.WorkshopResult.WORKSHOP_LIMIT_BATCH_COUNT, itemID, batchCount)
            return
        
        ret = self.doWorkshopManufactoring(itemID, batchCount)

        self.client.onWorkshopMF(ret, itemID, batchCount)

    #------------------------------------------------client api------------------------------------------------------------------

    #------------------------------------------------workshop biz----------------------------------------------------------------

    def doWorkshopManufactoring(self, itemID, batchCount):
        # 消耗的道具
        costItems = {}
        # 消耗的货币
        costCurrency = {}
        ...
        # 中间产出的需扣除道具
        costExtraItems = {}
        ret = self.calculateWorkshopMaterials(itemID, batchCount, costCurrency, costItems, outItems, costExtraItems)
    ...
    def calculateWorkshopMaterials(self, itemID, batchCount, costCurrency, costItems, outItems, costExtraItems):
        DEBUG_MSG("calculateWorkshopMaterials ", itemID, batchCount, costCurrency, outItems, costItems, costExtraItems)
        produceCfgData = WSP.datas.get(itemID)
        if not produceCfgData:
        ...
        ret = self.doWorkshopCalculation(produceCfgData, itemID, batchCount, costCurrency, costItems, outItems, costExtraItems)
        
        return ret
    
    def doWorkshopCalculation(self, produceCfgData, itemID, batchCount, costCurrency, costItems, outItems, costExtraItems):
        costs = produceCfgData['cost']
        materials = produceCfgData['materials']
        unboundProb = produceCfgData['unboundProb']
        for cost in costs:
        ...
        for material in materials:
            ...
            if mCount > remainCount:
                ...
                mCount -= remainCount
                ret = self.calculateWorkshopMaterials(mID, mCount, costCurrency, costItems, outItems, costExtraItems)
                if ret != gameconst.WorkshopResult.WORKSHOP_SUCCESS:
