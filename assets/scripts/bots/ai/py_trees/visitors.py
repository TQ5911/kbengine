...
class VisitorBase(object):
    ...
    def run(self, behaviour, treeOwner):
        """
...
class DebugVisitor(VisitorBase):
    ...
    def run(self, behaviour, treeOwner):
        if behaviour.feedback_message:
            behaviour.logger.debug("%s.run() [%s][%s]" % (self.__class__.__name__, behaviour.feedback_message, behaviour.getStatus(treeOwner)))
        else:
            behaviour.logger.debug("%s.run() [%s]" % (self.__class__.__name__, behaviour.getStatus(treeOwner)))
...
class SnapshotVisitor(VisitorBase):
    ...
    def run(self, behaviour, treeOwner):
        ...
        Args:
            behaviour (:class:`~py_trees.behaviour.Behaviour`): behaviour that is ticking
        """
        self.nodes[behaviour.id] = behaviour.getStatus(treeOwner)
        if behaviour.getStatus(treeOwner) == common.Status.RUNNING:
