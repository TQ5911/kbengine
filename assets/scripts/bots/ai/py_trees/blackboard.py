...
class CheckBlackboardVariable(behaviours.Behaviour):
    ...
    def terminate(self, treeOwner, new_status):
        ...
        """
        self.logger.debug("%s.terminate(%s)" % (self.__class__.__name__, "%s->%s" % (self.getStatus(treeOwner), new_status) if self.getStatus(treeOwner) != new_status else "%s" % new_status))
