...
class Composite(Behaviour):
    ...
    def stop(self, treeOwner, new_status=Status.INVALID):
        """
        ...
        """
        curState = self.getStatus(treeOwner)
        self.logger.debug("%s.stop()[%s]" % (self.__class__.__name__, "%s->%s" % (curState, new_status) if curState != new_status else "%s" % new_status))
        if new_status == Status.INVALID:
            for child in self.children:
                child.stop(treeOwner, new_status)
        ...
        self.terminate(treeOwner, new_status)
        self.setStatus(treeOwner, new_status)
        #self.iterator = self.tick()

    def tip(self, treeOwner):
        """
        ...
        """
        return self.current_child(treeOwner).tip(treeOwner) if self.current_child(treeOwner) is not None else None
    ...
    def add_child(self, child):
        ...
        child.parent = self
        child.treeLevel = self.treeLevel+1
        return child.id
    ...
    def remove_child(self, treeOwner, child):
        """
        ...
        """
        if child.getStatus(treeOwner) == Status.RUNNING:
            child.stop(treeOwner, Status.INVALID)
        child_index = self.children.index(child)
        self.children.remove(child)
        return child_index

    def remove_all_children(self, treeOwner):
        """
    ...
    def replace_child(self, treeOwner, child, replacement):
        """
    ...
    def remove_child_by_id(self, treeOwner, child_id):
        """
        ...
        if child is not None:
            if child.getStatus(treeOwner) == Status.RUNNING:
                child.stop(treeOwner, Status.INVALID)
            self.children.remove(treeOwner, child)
        else:
    ...
    def reloadScript(self):
        super(Composite, self).reloadScript()
...
class Selector(Composite):
    ...
    def tick(self, treeOwner):
        ...
        """
        curStatus = self.getStatus(treeOwner)
        isEvaluate = self.evaluate(treeOwner)
        self.traceLog(treeOwner, '%s START tick: %s %s %s %s %s'%('++++++'*self.treeLevel, self.getExecuteCount(treeOwner),self.executeMaxCount, self.__class__.__name__, self.name, curStatus))

        if curStatus != Status.RUNNING and not isEvaluate:  # 增加condition判断是否可以执行
            self.setStatus(treeOwner, Status.FAILURE)
            self.end_tick(treeOwner)
            return

        if self.executeMaxCount and self.executeMaxCount <= self.getExecuteCount(treeOwner):
            self.setStatus(treeOwner, Status.FAILURE)
            self.end_tick(treeOwner)
            return
        ...
        if curStatus == Status.RUNNING or isEvaluate:  # 增加condition判断是否可以执行
            self.update(treeOwner)

        curStatus = self.getStatus(treeOwner)
        currentChild = self.current_child(treeOwner)
        if currentChild is not None and curStatus == Status.RUNNING:
            # run our child, and invalidate anyone else who may have been ticked last run
            # (bit wasteful always checking for the latter)
            bTick = False
            childIndex = 0
            for child in self.children:
                if child is currentChild or bTick:
                    if child is currentChild:
                        child.tick(treeOwner)
                        childStatus = child.getStatus(treeOwner)
                        if childStatus == Status.RUNNING or childStatus == Status.SUCCESS:
                            self.setStatus(treeOwner, childStatus)
                            if childStatus == Status.RUNNING:
                                self.setCurrentIndex(treeOwner, childIndex)
                            self.end_tick(treeOwner)
                            return
                        bTick = True
                    else:
            ...
            # previous = self.current_child(treeOwner)
            childIndex = 0
            for child in self.children:
                child.tick(treeOwner)
                childStatus = child.getStatus(treeOwner)
                if childStatus == Status.RUNNING or childStatus == Status.SUCCESS:
                    ...
                    if currentChild is None or currentChild != self.current_child(treeOwner):
                        # we interrupted, invalidate everything at a lower priority
                        passed = False
                        for childs in self.children:
                            if passed:
                                if childs.getStatus(treeOwner) != Status.INVALID:
                                    childs.stop(treeOwner, Status.INVALID)
                            passed = True if childs == self.current_child(treeOwner) else passed
                    self.end_tick(treeOwner)
                    return
                childIndex += 1
    ...
    def current_child(self, treeOwner):
        """
        ...
        """
        curentIndex = self.getCurrentIndex(treeOwner)
        if curentIndex == -1:
            return None
        return self.children[curentIndex] if self.children else None
    ...
    def stop(self, treeOwner, new_status=Status.INVALID):
        ...
        # SUCCESS or FAILURE
        if new_status == Status.INVALID:
            self.setCurrentIndex(treeOwner, -1)

        Composite.stop(self, treeOwner, new_status)
...
class Chooser(Selector):
    ...
    def tick(self, treeOwner):
        ...
        """
        curState = self.getStatus(treeOwner)
        self.traceLog(treeOwner, '%s START tick: %s %s %s %s %s'%('++++++'*self.treeLevel, self.getExecuteCount(treeOwner),self.executeMaxCount, self.__class__.__name__, self.name, curState))

        if curState != Status.RUNNING and not self.evaluate(treeOwner):  # 增加condition判断是否可以执行
            self.setStatus(treeOwner, Status.FAILURE)
            yield from self.end_tick(treeOwner)
            return
        ...
        if curState != Status.RUNNING:
            ...
            self.initialise()
        # run any work designated by a customised instance of this class
        if curState == Status.RUNNING or self.evaluate(treeOwner):  # 增加condition判断是否可以执行
            self.update(treeOwner)
        if self.current_child(treeOwner) is not None:
            # run our child, and invalidate anyone else who may have been ticked last run
            # (bit wasteful always checking for the latter)
            for child in self.children:
                if child is self.current_child(treeOwner):
                    for node in self.current_child(treeOwner).tick(treeOwner):
                        nodeStatus = node.getStatus(treeOwner)
                        if nodeStatus == Status.RUNNING or nodeStatus == Status.SUCCESS:
                            self.setStatus(treeOwner, nodeStatus)
                            yield from self.end_tick(treeOwner)
                            return
                        yield node
                elif child.getStatus(treeOwner) != Status.INVALID:
                    child.stop(treeOwner, Status.INVALID)
        else:
            for idx, child in enumerate(self.children):
                for node in child.tick(treeOwner):
                    yield node
                childStatus = child.getStatus(treeOwner)
                if childStatus == Status.RUNNING or childStatus == Status.SUCCESS:
                    self.setCurrentIndex(treeOwner, idx)
                    break
        new_status = self.current_child(treeOwner).getStatus(treeOwner) if self.current_child(treeOwner) is not None else Status.FAILURE
        self.stop(treeOwner, new_status)
        yield from self.end_tick(treeOwner)
...
class Sequence(Composite):
    ...
    def tick(self, treeOwner):
        ...
        if curStatus != Status.RUNNING:
            # self.logger.debug("%s.tick() [resetting]" % self.__class__.__name__, self.level)
            # sequence specific handling
            self.setCurrentIndex(treeOwner, 0)
            for child in self.children:
                # reset the children, this helps when introspecting the tree
                if child.getStatus(treeOwner) != Status.INVALID:
                    child.stop(treeOwner, Status.INVALID)
        ...
        if currentChild is not None and curStatus == Status.RUNNING:
            ...
            for child in self.children:
                if child is currentChild or bTick:
                    if child is currentChild:
                        child.tick(treeOwner)
                        childStatus = child.getStatus(treeOwner)
                        if childStatus == Status.RUNNING or childStatus == Status.FAILURE:
                            self.setStatus(treeOwner, childStatus)
                            if childStatus == Status.RUNNING:
                                self.setCurrentIndex(treeOwner, childIndex)
                            self.end_tick(treeOwner)
            ...
            self.setStatus(treeOwner, Status.INVALID)
            childIndex = 0
            for child in self.children:
                child.tick(treeOwner)
                childStatus = child.getStatus(treeOwner)
        ...
        curIndex = self.getCurrentIndex(treeOwner)
        self.setCurrentIndex(treeOwner, curIndex-1)
        self.stop(treeOwner, Status.SUCCESS)
        self.end_tick(treeOwner)
        return
    ...
    def current_child(self, treeOwner):
        ...
        Returns:
            :class:`~py_trees.behaviour.Behaviour`: the child that is currently running, or None
        """
        if self.getCurrentIndex(treeOwner) == -1:
            return None
        return self.children[self.getCurrentIndex(treeOwner)] if self.children else None
...
class Parallel(Composite):
    ...
    * Parallels with policy :data:`~py_trees.common.ParallelPolicy.SUCCESS_ON_ALL` only returns :py:data:`~py_trees.common.Status.SUCCESS` if **all** children return :py:data:`~py_trees.common.Status.SUCCESS`
    ...
    def tick(self, treeOwner):
        ...
        """
        curStatus = self.getStatus(treeOwner)
        if curStatus != Status.RUNNING and not self.evaluate(treeOwner):  # 增加condition判断是否可以执行
            self.setStatus(treeOwner, Status.FAILURE)
            self.end_tick(treeOwner)
            return
        ...
        if any([c.getStatus(treeOwner) == Status.RUNNING for c in self.children]):
            new_status = Status.RUNNING
        elif any([c.getStatus(treeOwner) == Status.FAILURE for c in self.children]):
            new_status = Status.FAILURE
        else:
            if self.policy == common.ParallelPolicy.SUCCESS_ON_ALL:
                if all([c.getStatus(treeOwner) == Status.SUCCESS for c in self.children]):
                    new_status = Status.SUCCESS
            elif self.policy == common.ParallelPolicy.SUCCESS_ON_ONE:
                if any([c.getStatus(treeOwner) == Status.SUCCESS for c in self.children]):
                    new_status = Status.SUCCESS
    ...
    def stop(self, treeOwner, new_status=Status.INVALID):
        ...
        # retain information about the last running child if the new status is
        # SUCCESS or FAILURE
        super(Parallel, self).stop(treeOwner, new_status)
        if new_status == Status.INVALID:
            self.setCurrentIndex(treeOwner, -1)
##############################################################################
# Condition
...
class ConditionNOT(Condition):
    def __init__(self, conditionObj = None, *args, **kwargs):
        self.name = 'ConditionNOT'
        self.conditionObj = conditionObj

    def externalCondition(self, treeOwner):
        if not self.conditionObj:
            return False
        if self.conditionObj and hasattr(self.conditionObj, 'externalCondition'):
            return not getattr(self.conditionObj, 'externalCondition')()
