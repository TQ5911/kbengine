...
def create_imposter(cls):
    ...
    class Imposter(behaviour.Behaviour):
        ...
        def tip(self, treeOwner):
            ...
            """
            if isinstance(self.original, composites.Composite):
                return self.original.tip(treeOwner)
            else:
                return super(Imposter, self).tip(treeOwner)
        ...
        def tick(self, treeOwner):
            ...
            if self.getStatus(treeOwner) != common.Status.RUNNING:
                self.initialise()

            # initialise() and terminate() for the original behaviour
            # will be called from inside the original's tick()
            for behaviour in self.original.tick(treeOwner):
                if behaviour != self.original:
                    yield behaviour
            # new_status = self.update()
            if self.getStatus(treeOwner) != common.Status.RUNNING and not self.evaluate(treeOwner):  # 增加condition判断是否可以执行
        ...
        def update(self, treeOwner):
            ...
            the meta classes.
            """
            return self.original.getStatus(treeOwner)
...
def _oneshot_tick(func):
    ...
    @functools.wraps(func)
    def wrapped(self, treeOwner, *args, **kwargs):
        if self.getStatus(treeOwner) == common.Status.FAILURE or self.getStatus(treeOwner) == common.Status.SUCCESS:
...
def failure_is_success(cls):
    ...
    def _update(func):
        @functools.wraps(func)
        def wrapped(self, treeOwner):
            if self.original.getStatus(treeOwner) == common.Status.FAILURE:
...
def success_is_failure(cls):
    ...
    def _update(func):
        @functools.wraps(func)
        def wrapped(self, treeOwner):
            if self.original.getStatus(treeOwner) == common.Status.SUCCESS:
...
def condition(cls, status):
    ...
    def _update(func):
        ...
        def wrapped(self, treeOwner):
            ...
            self.feedback_message = "'{0}' has status {1}, waiting for {2}".format(self.original.name, self.original.getStatus(treeOwner), self.succeed_status)
            if self.original.getStatus(treeOwner) == self.succeed_status:
                if self.original.getStatus(treeOwner) == common.Status.RUNNING:
                    self.original.stop(treeOwner)
                return common.Status.SUCCESS
            else:
                return common.Status.RUNNING
        return wrapped
