...
class BehaviourTree(object):
    ...
    * Continuous tick-tock support
    ...
    def tick(self, pre_tick_handler=None, post_tick_handler=None, treeOwner=None):
        """
        ...
        for visitor in self.visitors:
            visitor.initialise()
        # tick
        for node in self.root.tick(treeOwner):
    ...
    def tick_tock(self, sleep_ms, number_of_iterations=CONTINUOUS_TICK_TOCK, pre_tick_handler=None, post_tick_handler=None, treeOwner=None):
        """
        ...
        while not self.interrupt_tick_tocking and (tick_tocks < number_of_iterations or number_of_iterations == CONTINUOUS_TICK_TOCK):
            self.tick(pre_tick_handler, post_tick_handler, treeOwner)
            try:
    ...
    def tip(self, treeOwner):
        ...
        """
        return self.root.tip(treeOwner)
    ...
    def destroy(self, treeOwner):
        """
        Destroy the tree by stopping the root node.
        """
        self.root.stop(treeOwner)
