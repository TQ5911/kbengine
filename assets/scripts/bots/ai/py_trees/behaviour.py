...
class Behaviour(object):
    ...
    def __init__(self, name="", *args, **kwargs):
        ...
        self.conditionList = []
        self.logLevel = logging.Level.INFO
        self.treeLevel = 0
        self.executeMaxCount = 0
        self.logger = logging.Logger(name, self)

    ############################################
    # User Customisable Functions (virtual)
    ############################################

    def setStatus(self, treeOwner, status):
        treeOwner.setStatus(self.id, status)
        if status == Status.SUCCESS:
            treeOwner.addExecuteCount(self.id)

    def getStatus(self, treeOwner):
        return treeOwner.getStatus(self.id)

    def getExecuteCount(self, treeOwner):
        return treeOwner.getExecuteCount(self.id)

    def setCurrentIndex(self, treeOwner, currentIndex):
        treeOwner.setCurrentIndex(self.id, currentIndex)

    def getCurrentIndex(self, treeOwner):
        return treeOwner.getCurrentIndex(self.id)
    ...
    def terminate(self, treeOwner, new_status):
        """
        .. note:: User Customisable Callback
    ...
    def tick_once(self, treeOwner):
        ...
        self.traceLog(treeOwner, 'tick begin')
        self.tick(treeOwner)
    ...
    def tip(self, treeOwner):
        ...
        """
        return self if self.getStatus(treeOwner) != Status.INVALID else None
    ...
    def tick(self, treeOwner):
        ...
        if curStatus != Status.RUNNING and not self.evaluate(treeOwner): # 增加condition判断是否可以执行
            ...
            new_status = self.update(treeOwner)
        ...
        self.traceLog(treeOwner, '%sSTART stick: %s %s %s'%('++++++'*self.treeLevel, self.__class__.__name__, self.name, self.getStatus(treeOwner)))

        self.end_tick(treeOwner)
        return

    def traceLog(self, treeOwner, msg):
        if getattr(treeOwner, 'logAITrace', 0):
    ...
    def stop(self, treeOwner, new_status=Status.INVALID):
        ...
        """
        status = self.getStatus(treeOwner)
        self.logger.debug("%s.stop(%s)" % (self.__class__.__name__, "%s->%s" % (status, new_status) if status != new_status else "%s" % new_status))
        self.terminate(treeOwner, new_status)
        self.setStatus(treeOwner, new_status)
        #self.iterator = self.tick()

    def addCondition(self, preCondition):
        if preCondition not in self.conditionList:
            self.conditionList.append(preCondition)

    def removeCodition(self, preCondition):
        if preCondition in self.conditionList:
            self.conditionList.remove(preCondition)

    def setExecuteMaxCount(self, maxCount):
        self.executeMaxCount = maxCount

    def evaluate(self, treeOwner):
        for condition in self.conditionList:
            if condition:
                if condition.externalCondition and not condition.externalCondition(treeOwner):
                    return False
        return True

    def setLogLevel(self, level):
        self.logLevel = logging.Level.getRealLevel(level)
        for child in self.children:
            child.setLogLevel(level)

    def setId(self, startId):
        self.id = startId
        self.setChildId(startId)

    def setChildId(self, startId):
        for c in self.children:
            startId += 1
            c.id = startId
        for c in  self.children:
            startId = c.setChildId(startId)
        return startId

    def end_tick(self, treeOwner):
        self.traceLog(treeOwner, '%s END tick: %s %s %s %s %s'%('++++++'*self.treeLevel,self.getExecuteCount(treeOwner),self.executeMaxCount, self.__class__.__name__, self.name, self.getStatus(treeOwner)))
