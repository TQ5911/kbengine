...
import message_battleLog_def as MBD
...
class SkillManager(iCell.ICell, iEventActions.IEventActions, iFlowController.IFlowController,
    ...
    def onScriptSetAttr(self, key, value, isInit):
        ...
        if self.IsAvatar:
            formulaStr = 'formulaPlayer'
        elif self.IsAvatarMirror and self.isNeedInheritOwnerProp():  # 注意现在有些特定的avatarMirror对象走怪物的属性计算公式
            formulaStr = 'formulaRobot'
        elif self.IsPet or (self.IsPlunderPet and self.monsterHomeInfo.extraProps):
            formulaStr = 'formulaLingShou'
        else:
            formulaStr = 'formulaMonster'
        ...
        if FDD.datas[key][formulaStr]:
            ...
            for i in range(nDepth):
                ...
                if f.f_code.co_name in (
                        '__init__', 'checkOnUpdateProp', 'endTransform', 'initBaseProperties', 'inheritProps'):
                    return
    ...
    def _immuneDeathFinished(self, srcEntId, context):
        ...
        if iInfo.deadAfterImmunning:
            ...
            if self.hasPropTransformState():
                self.endTransform()

            if self.hasState(gameconst.State.huanhua):
                self.endHuanhua()
    ...
    def killSelf(self):
        ...
        if iInfo:
            iInfo.status = gameconst.ImmuneDeathState.IMMUNE_FINISHED
        self.modifyHP(-self.hp, self.id, gameconst.SourceType.Default, 0)
        self._endBigWorldDuel(self)
    ...
    def modifyHP(self, hpVal, releaseRoleId, srcType, srcId, forceDead=False, context=None):
        ...
        if iInfo:
            ...
            if not forceDead and curHp <= 0 and iInfo.status == gameconst.ImmuneDeathState.IMMUNE_VALID:
                ...
                if self.IsAvatar and realReleaseRole.IsAvatar and not self.hasTempMiscProp(gameconst.AvatarProps.isFenxinPointChanged):
                    self.setTempMiscProp(gameconst.AvatarProps.isFenxinPointChanged, realReleaseRole.isFenxinPointChanged(self))
        ...
        # bInfo = self.bigWorldDuelCacheInfo if self.IsAvatar else None
        # tbInfo = realReleaseRole.bigWorldDuelCacheInfo if realReleaseRole.IsAvatar else None
        # if curHp <= 0 and bInfo and tbInfo and bInfo.isInDuel() and tbInfo.isInDuel() \
        #         and bInfo.duelUUID == tbInfo.duelUUID:
        #     # 【【任务】支持boss濒死】
        #     # https://www.tapd.cn/57153713/prong/stories/view/1157153713001007833
        #     # 修改curHp: 兼容原代码逻辑; 修改self.hp: 锁血相关, 需要将self.hp 修改提前到最先, 这里需求强制再赋值
        #
        #     self.hp = curHp = 1
        #     bInfo.duelStatus = gameconst.BigWorldDuelStatus.END
        #     self.bigWorldDuelCacheInfo = self.bigWorldDuelCacheInfo
            # duelFlagEntId = self.getTempMiscProp(gameconst.AvatarProps.bigWorldDuelTempCacheVal).duelFlagEntId
            # duelFlagEnt = KBEngine.entities.get(duelFlagEntId)
            # if not duelFlagEnt:
            #     ERROR_MSG('modifyHP:: duelFlagEntity not found', duelFlagEntId)
            # else:
            #     duelFlagEnt.onAvatarFailedInBigWorldDuel(self.id, bInfo.duelSide)
    ...
    def _isLoveSkillApplicable(self, marriageGBID):
        if marriageGBID not in self.teamInfo.onlineMembers():
            return False

        now = utils.getNow()
        if self.loveSkillTS >= now:
            return False

        marriageEntID = self.teamInfo.getPlayerBoxbyGbId(marriageGBID).id
        ent = KBEngine.entities.get(marriageEntID)
        if not ent:
            return False

        if self.spaceNo != ent.spaceNo or sMath.distance2DToCompareFrom3DPosition(self.position, ent.position) > pow(
                RCMC.datas['cpSkillRange']['value'], 2):
            return False
    ...
    def _useSkillBySkillObj(self, skill, targetID, arr, isClient=False, compensateTime=0):
        ...
        if ret != gameconst.UseSkillCheck.CHEKC_OK:
            ERROR_MSG("Spell::doUseSkill(%i):skillID=%i ret=%i" % (self.id, skillId, ret))
        ...
        print(f'/////////////////////isClient:{isClient},realSkill.checkSkillArgs(arr):{realSkill.checkSkillArgs(arr)}')
        ...
        if skill.hasTag(gameconst.SkillTag.UltraSkill):
    ...
    def doSkillAction(self, skillId, context, calcDelay, needUseCheck=False, checkIgnoreReasons=0, duration=0,
        ...
        if skill.getEffectTarget(skill.skillId) == 'None':
            ...
            if needUseCheck:
                if effectedTargets:
                    for target in effectedTargets:
                        ...
                        # self.allClients.onMultiAttackSkillBreak(skill.skillId, context.actionStage)
                        skill.onSkillActionFinished(self, context.useTargetId, skillArgs, context, calcDelay, duration)
                        return actionFinished

            skillSchoolTag = skill.getClassTag(self)
            isAttackSkill = skill.isAttackSkill(skill.skillId)
            isSelfDie = self.isDie()
            # isSetFightState = False
            for target in effectedTargets:
                if target.isDie() or target.isDestroyed:
                    continue

                # if not isSetFightState and self.id != target.id and target.hasState(
                #         gameconst.State.Fighting) and not isSelfDie:
                #     self.setState(gameconst.State.Fighting)
                #     isSetFightState = True
            ...
            if isAttackSkill and skill.hasTag(gameconst.SkillTag.GeneralSkill) and context.useTargetId:
                target = KBEngine.entities.get(context.useTargetId)
                if not target or target.isDie() or target.isDestroyed:
                    generalSkillReset = True

        if skillDamges.damageInfo and not self.isDestroyed:
            self.sendSkillDamage(skillDamges)
            skillDamges.damageInfo = []

        if generalSkillReset:
            skill.resetSkill(self, reason=gameconst.ResetSkillReason.GeneralSkillBreak, doRemoveState=True)
        if actionFinished:
            skill.onSkillActionFinished(self, context.useTargetId, skillArgs, context, calcDelay, duration,
                                        doRemoveState)
    ...
    def applyDmgActionResult(self, target, context, dmgResult):
        ...
        if realDmgVal >= 0:
            recordDmgVal = abs(
                target.modifyHP(-realDmgVal, dmgSrcEnt.id, context.getDmgSourceType(), context.getDmgSourceId(),
                                context=context))
    ...
    def applyHealActionResult(self, target, context, healResult: combatSkill.HealResult):
        ...
        if healResult.healVal > 0:
            if target:
                ...
                hpDelta = target.modifyHP(healResult.healVal, self.id, context.getDmgSourceType(),
                                          context.getDmgSourceId())
                if hpDelta:
            ...
            if healResult.isCrit:
                skillDamges.damageInfo.append(
                    combatSkill.SkillDamageVal(target.id, healResult.healVal, gameconst.HitType.HealCrit))
            else:
                skillDamges.damageInfo.append(
                    combatSkill.SkillDamageVal(target.id, healResult.healVal, gameconst.HitType.Heal))

        self.calcHealStats(hpDelta)
        self.sendHealMsgs(target, context, healResult)

    def sendHealMsgs(self, target, context, healResult):
        ctx = context or context.parentContext

        if ctx.actionType == actionContext.ACTION_USE_SKILL:
            skillName = ctx.skillObj.getSkillName(ctx.skillObj.skillId)
            # if healResult.isCrit:
            #     self.sendCombatMsg(MBD.datas.fatalHeal, [skillName, target.name, healResult.healVal])
            #     self.id != target.id and target.sendCombatMsg(MBD.datas.beFatalHealed,
            #                                                   [self.name, skillName, healResult.healVal])
            # else:
            #     self.sendCombatMsg(MBD.datas.heal, [skillName, target.name, healResult.healVal])
            #     self.id != target.id and target.sendCombatMsg(MBD.datas.beHealed,
            #                                                   [self.name, skillName, healResult.healVal])
        elif ctx.actionType in (actionContext.ACTION_BUFF_END, actionContext.ACTION_BUFF_TICK):
            buffName = buff.Buff.getBuffName(ctx.buffId)
            # if healResult.isCrit:
            #     self.sendCombatMsg(MBD.datas.targetGetBuffFatalDamage,
            #                        [ctx.buffLevel, buffName, target.name, healResult.healVal])
            #     self.id != target.id and target.sendCombatMsg(MBD.datas.getBuffFatalDamage,
            #                                                   [ctx.buffLevel, buffName, healResult.healVal])
            # else:
            #     self.sendCombatMsg(MBD.datas.targetGetBuffHeal,
            #                        [ctx.buffLevel, buffName, target.name, healResult.healVal])
            #     self.id != target.id and target.sendCombatMsg(MBD.datas.getBuffHeal,
            #                                                   [ctx.buffLevel, buffName, healResult.healVal])
    ...
    def getCreationCombatProps(self):
        ...
        inheritList = ['baseMinPhysicalAtk', 'adjMinPhysicalAtk', 'adjMinPhysicalAtkAbs', 'baseMaxPhysicalAtk',
                       ...
                       'adjFatal', 'baseMortal', 'adjMortal', 'adjBash', 'baseBossDmg', 'adjBossDmg', 'baseStunEnh',
                       'adjStunEnh', 'baseSilentEnh', 'adjSilentEnh', 'baseKnockEnh', 'adjKnockEnh', 'baseDebilityEnh',
                       'adjDebilityEnh', 'baseFrozenEnh', 'adjFrozenEnh', 'baseBreakShieldEnh', 'adjBreakShieldEnh'
                       ]
    ...
    def getSummonCloneCombatProps(self):
        ...
        inheritList = ['baseFullHp', 'adjFullHp', 'adjFullHpAbs', 'mulFullHp', 'minPhysicalAtk', 'baseMinPhysicalAtk',
                       ...
                       'maxMagicAtk', 'baseMaxMagicAtk', 'adjMaxMagicAtk', 'adjMaxMagicAtkAbs', 'minPhysicalArmor',
                       'baseMinPhysicalArmor', 'adjMinPhysicalArmor', 'adjMinPhysicalArmorAbs', 'maxPhysicalArmor',
                       'baseMaxPhysicalArmor', 'adjMaxPhysicalArmor', 'adjMaxPhysicalArmorAbs', 'minMagicArmor',
                       'baseMinMagicArmor', 'adjMinMagicArmor', 'adjMinMagicArmorAbs', 'maxMagicArmor', 'baseMaxMagicArmor',
                       'adjMaxMagicArmor', 'adjMaxMagicArmorAbs', 'baseHit', 'adjHit', 'mulHit', 'baseDodge', 'adjDodge',
                       'mulDodge',
    ...
    def getTargetIdsByTargetType(self, targetString):
        ...
        if targetString == "None" or targetString == "Any" or targetString == "AnyExGB" or targetString == "Monster":
            entityIds = list(self.enemyCacheSet.union(self.notEnemyCacheSet, {self.id}))
        elif targetString == "Enemy" or targetString == "PlayerEnemy" or targetString == "EnemyExTarget":
            entityIds = list(self.enemyCacheSet)
        elif targetString == "Friend" or targetString == "FriendExGB":
            entityIds = list(self.friendCacheSet | {self.id})
        elif targetString == "Self":
            entityIds = [self.id]
        elif targetString == "FriendExS" or targetString == "CorpsePlayerExS" or targetString == "FriendPlayerExS":
            entityIds = list(self.friendCacheSet)
        elif targetString == "CorpseEnemy":
            entityIds = list(self.enemyCacheSet)
        elif targetString == "PlayerExS" or targetString == "AnyExs":
            entityIds = list(self.enemyCacheSet.union(self.notEnemyCacheSet))
    ...
    def getTargetsWithNum(self, centerEnt, targetId, targetString, targetRange, targetNum, checkScopeFun,
        ...
        for eId in entityIds:
            entity = KBEngine.entities.get(eId)
            if self.isTarget(target, eId, targetString, targetRange, checkScopeFun, beginSkillPosition):
    ...
    def onChongfengMoveOver(self, isSucc):
        ...
        if not skill:
            ERROR_MSG('delayCalcSkill: cannot get skill', skill.skillId, context)
            self.endChongfeng(None, 0, [])
            return
    ...
    # def decreaseZedPoint(self):
    #     self.popTempMiscProp(gameconst.AvatarProps.zedPointDecTimer, 0)
    #     if self.zedPoint > 0:
    #         self.zedPoint -= 1
    #
    #     if self.zedPoint > 0:
    #         decreaseTimer = self._callback(CONST.datas['zedAutoDeductTime']['value'], 'decreaseZedPoint', (),
    #                                        gametimer.TIMER_TAG_DECREASE_ZED_POINT, '', 'popTempMiscProp',
    #                                        (gameconst.AvatarProps.zedPointDecTimer,))
    #         self.setTempMiscProp(gameconst.AvatarProps.zedPointDecTimer, decreaseTimer)
    ...
    def reliveToPos(self, pos, hp):
        ...
        """
        self._relive()
        self.addBuff(CONST.datas["resurrectProtectBuffID"]["value"], 1, self.id)
        newHp = hp
        if hp == 0:
    ...
    def getDefaultReliveHp(self):
        return max(min(round(dataUtils.getConstVal('resurrectSituHP', 20) * 0.01 * self.fullHp), self.fullHp), 1)
    ...
    def deathCreateCollection(self, radius, numProb, collectionIdProb, disappearTime):
        ...
        for i in range(createNum):
            ...
            pos_ = self.getRandomPosition(self.position, radius)
