...
class TimedIntervalsEntityVal(userType.UserSingleType):
    ...
    def __str__(self):
        return f'TimedIntervalsEntityVal(refreshTimedID={self.refreshTimedID}, nextStartTime={self.nextStartTime}, interval={self.interval}, refreshDataList={self.refreshDataList}, srcRefreshDataList={self.srcRefreshDataList}, nextStartTimeStr={utils.getCommonTimeStrFromTimeStamp(self.nextStartTime)})'

class TimeLimitedEntityVal(userType.UserSingleType):
    ...
    def __str__(self):
        return f'TimeLimitedEntityVal(refreshTimedID={self.refreshTimedID}, stageType={self.stageType}, nextStartTime={self.nextStartTime}, interval={self.interval}, refreshDataList={self.refreshDataList}, srcRefreshDataList={self.srcRefreshDataList}, nextStartTimeStr={utils.getCommonTimeStrFromTimeStamp(self.nextStartTime)})'
...
class ITimerEntityRefresh(object):
    ...
    def initTimerEntities(self, space, readyTimerEntitiesMap):
        ...
        for refreshTimedID, refreshDataList in readyTimerEntitiesMap.items():
            ...
            if not refreshCfg:
                LOG_WARN("initTimerEntities not refreshCfg")
                continue
            ...
            if refreshType == gameconst.TimerEntityRefreshType.TIMED_INTERVALS:
                ...
                LOG_WARN("initTimerEntities RefreshType error", refreshType)
                continue
...
########################################################################################################
    ...
    def doTimedIntervalsEntityDestroy(self, timedIntervalsEntityVal):
        LOG_DBG("doTimedIntervalsEntityDestroy1", timedIntervalsEntityVal)
        for refreshData in timedIntervalsEntityVal.refreshDataList:
            (id_, entityType, extraData) = refreshData
            entityID = extraData["EntityID"]
            ents = self.listEntitiesByTag('gid_{}'.format(id_))
            LOG_DBG('doTimedIntervalsEntityDestroy2', 'gid_{}'.format(id_), len(ents))
...
########################################################################################################
    ...
    def onTimeLimitedEntityRefreshTimerCallback(self):
        ...
        if not refreshCfg:
            LOG_WARN("onTimeLimitedEntityRefreshTimerCallback not refreshCfg")
            return
    ...
    def doTimeLimitedEntityDestroy(self, timeLimitedEntityVal):
        LOG_DBG("doTimeLimitedEntityDestroy1", timeLimitedEntityVal)
        for refreshData in timeLimitedEntityVal.refreshDataList:
            (id_, entityType, extraData) = refreshData
            entityID = extraData["EntityID"]
            count_ = extraData["RefreshNum"]
            ents = self.listEntitiesByTag('gid_{}'.format(id_))
            LOG_DBG('doTimeLimitedEntityDestroy2', 'gid_{}'.format(id_), entityID, len(ents))
    ...
    def doTimeLimitedGroupEntityDestroy(self, info):
        LOG_DBG("doTimeLimitedGroupEntityDestroy1", info)
        id_ = info['id']
        entityID = info["eId"]
        ents = self.listEntitiesByTag('gid_{}'.format(id_))
        LOG_DBG('doTimeLimitedGroupEntityDestroy2', 'gid_{}'.format(id_), entityID, len(ents))
...
########################################################################################################
    ...
    def onDestroyGroupEntities(self, info):
        ...
        if refreshType == gameconst.TimerEntityRefreshType.TIMED_INTERVALS:
            LOG_ERR("onDestroyGroupEntities error")
        ...
        for ent in ents:
            if ent.IsMonster:
                if creep_base.datas.get(entityID, {}).get('type', 0) == gameconst.MonsterType.ADVANCE:
                    ...
                    ent.onEntityRefresh()
                ent.safeDestroy()
                #ent.killSelf(gameconst.SourceType.SrcTpDefault)
    ...
    def onTimerEntityRefresh(self, refreshData):
        ...
        if entityType == gameconst.EntityType.MONSTER and creep_base.datas.get(entityID, {}).get('type', 0) == gameconst.MonsterType.ADVANCE:
            ...
            if not func:
                LOG_ERR("doTimedIntervalsEntityRefresh not WorldLineSpaceMgr", id_)
            else:
                func()
        else:
        ...
        if gid in self.refreshGID2TimerListMap[gameconst.TimerEntityRefreshType.TIME_LIMITED]:
            if timerId and timerId in self.refreshGID2TimerListMap[gameconst.TimerEntityRefreshType.TIME_LIMITED][gid]:
                ...
                for tId in self.refreshGID2TimerListMap[gameconst.TimerEntityRefreshType.TIME_LIMITED][gid]:
                    self.getCurrentSpace().cancelTimerCB(tId, gametimer.TIMER_TAG_SPACE_DO_REFRESH)
