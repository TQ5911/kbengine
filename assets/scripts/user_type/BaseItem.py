...
class PureItem(userType.UserSingleType):
    def __init__(self, dataDict):
        self.__dict__['itemId'] = dataDict['itemId']
        self.__dict__['itemNum'] = dataDict['itemNum']
        self.__dict__['uniqueId'] = dataDict['uniqueId']
        self.__dict__['bindType'] = dataDict['bindType']

        itemType, itemSubType = itemFactory.ItemFactory.getItemTypes(dataDict['itemId'])
        self.__dict__['itemType'] = itemType
        self.__dict__['itemSubType'] = itemSubType

        self.__dict__['createTime'] = dataDict['createTime']

        itemData = dataUtils.getCommItemData(dataDict['itemId'])
        ...
        expireTime = 0
        if expirationDate:
            expireTime = dataDict.get('expireTime', 0)
        elif expireTimeStr:
        ...
        self.__dict__['enableTime'] = dataDict.get('enableTime', enableTime)
        self.__dict__['expireTime'] = expireTime
        self.__dict__['attrJson'] = dataDict['attrJson']
        self.__dict__['lockStatus'] = dataDict.get('lockStatus', gameconst.ItemLockStatus.UNLOCKED)
    ...
    def __setattr__(self, attrName, value):
        if attrName == '__class__':
            ...
            # 又会执行 return setattr(self, attrName, value)，将 item的 __class__ 又重新设置为 PureItem
    ...
    def changeToSpecificItem(self):
        _itemClass = itemFactory.ItemFactory.getItemClass(self.__dict__['itemId'])
        try:
            object.__setattr__(self, '__class__', _itemClass)
...
class BaseItem(userType.UserSingleType, metaclass=abc.ABCMeta):
    ...
    def maxStackSize(itemId):
        ...
        return _itemData.get('maxStackSize')
    ...
    def __setstate__(self, stateDic):
        self.__init__(stateDic['itemId'], stateDic['itemNum'], bindType=stateDic['bindType'])
    ...
    def setItemNum(self, num):
        if num > self.maxStackSize(self.itemId) or num < 0:
            gameengine.panicStack('itemNum error', self.itemId, num, self.maxStackSize(self.itemId))
            return
        object.__setattr__(self, 'itemNum', num)
    ...
    def isSoul(self):
        return self.itemType == gameconst.ItemEnum.Normal and self.itemSubType in gameconst.ItemSubEnum.EQUIP_SOUL_TYPE
