...
function start_gameapp(){
    echo "interfaces start....."
    cid=`expr 300000 + $INST_ID`
    $KBE_BIN_PATH/interfaces --cid=$cid --instId=$INST_ID 2>&1 > /dev/null &
    echo "interfaces start done"
}

function stop_gameapp(){
    echo "interfaces stop...."
    pid=`cat $KBE_ROOT/assets/interfaces_${INST_ID}.pid`
    kill $pid
