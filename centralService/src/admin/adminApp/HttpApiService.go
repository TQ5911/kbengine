...
// handleGetAvatarServerId 通过玩家 gbId 查询 Redis 中 AvatarInfo_<gbId> 的 serverId 字段。
// 协议: 与 handlePlayerHttpCmdRequest 一致, Args 字段是 hex(gbId) 字符串。
// 返回: 成功时 body 形如 {"gbId":"...","serverId":20110};
//
//	玩家档案不存在 (Redis 里没这条 hash / 没有 serverId 字段) 时返回 HTTP_CMD_ERR_PARTITION_NOT_FOUND。
func (self *HttpCommandService) handleGetAvatarServerId(w http.ResponseWriter, req *http.Request) {
	appLog.Info("handleGetAvatarServerId")
	if req.Method != "POST" {
		w.WriteHeader(405)
		return
	}
	...
	if reqData.Args == "" {
		self.sendErrResponse(HTTP_CMD_ARGS_ERR, "args is empty, expected hex(gbId)", w)
		return
	}

	gbIdBytes, err := hex.DecodeString(reqData.Args)
	if err != nil {
		appLog.Errorf("handleGetAvatarServerId: hex decode args failed: %s raw=%s\n", err.Error(), reqData.Args)
		self.sendErrResponse(HTTP_CMD_ARGS_ERR, "invalid args, expected hex(gbId)", w)
		return
	}
	gbId := string(gbIdBytes)
	if gbId == "" {
		self.sendErrResponse(HTTP_CMD_ARGS_ERR, "gbId is empty after hex decode", w)
		return
	}

	serverId, err := common.GetAvatarServerId(self.app.redisPool, gbId)
	if err != nil {
		appLog.Errorf("handleGetAvatarServerId: query failed: %s gbId=%s\n", err.Error(), gbId)
		self.sendErrResponse(HTTP_CMD_ERR_INNER, "query serverId failed: "+err.Error(), w)
		return
	}
	if serverId == 0 {
		// key 或 serverId 字段不存在 — 玩家档案缺失/未初始化
		self.sendErrResponse(HTTP_CMD_ERR_PARTITION_NOT_FOUND,
			"player avatar info not found, gbId="+gbId, w)
		return
	}

	appLog.Infof("handleGetAvatarServerId ok, gbId=%s serverId=%d\n", gbId, serverId)
	...
	respBytes, err := json.Marshal(respData)
	if err != nil {
		appLog.Errorf("handleGetAvatarServerId: marshal response err: %s\n", err.Error())
		self.sendErrResponse(HTTP_CMD_ERR_DUMP_JSON, "marshal err", w)
		return
	}
...
func (self *HttpCommandService) startHttpApiServer(listenAddr string) {
	...
	http.HandleFunc("/getAvatarServerId", self.corsMiddleware(self.handleGetAvatarServerId))
